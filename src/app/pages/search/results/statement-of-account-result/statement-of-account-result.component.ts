...
import { AgedRecievablesActionsComponent } from '../../../billing/statement-of-account/statement-of-account-actions/statement-of-account-actions.component';
...
import { AgedRecievablesByCustomerSearchParams, AgedRecievablesSearchParams } from '../../../../_shared/models/invoiceParams.model';
import { SearchService } from '../../../../services/search.service';
import { ViewDetailsActionComponent } from '../view-details-action/view-details-action.component';

@Component({
  selector: 'ngx-statement-of-account-result',
  templateUrl: './statement-of-account-result.component.html',
  styleUrls: ['./statement-of-account-result.component.scss']
})
export class StatementOfAccountResultComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  initializeSource() {
    ...
    this.source.serviceCallBack = (params) => this._getStatementOfAccount(params).pipe(tap(x => this.searchService.notifyResultSetReady('StatementOfAccounts', x.data.length)));
    this.source.setSort([
  ...
  private _getStatementOfAccount(params: BaseParams): Observable<IPagination> {
    ...
    if (this.isSmartriseUser) {
      return this.invoiceService.searchAllAgedRecievablesBySmartriseUser(params as AgedRecievablesSearchParams);
    } else {
      return this.invoiceService.searchAllAgedRecievablesByCustomerUser(params as AgedRecievablesByCustomerSearchParams);
    }
  }

  private _fillSearchParameters(searchKey: string, sParam: AgedRecievablesSearchParams) {
